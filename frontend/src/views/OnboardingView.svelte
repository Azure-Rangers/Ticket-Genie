...
<div class="onboarding-view animate-fade">
  ...
  {#if mode === 'list'}
    ...
    <div class="card-box">
      ...
      {:else}
        <div class="case-list">
          {#each cases as item}
            <button class="case-row" on:click={() => openCase(item)}>
  ...
  {:else if mode === 'detail' && selectedCase}
    ...
    <div class="card-box">
      ...
      <div class="linked-tickets">{#each selectedCase.tickets as ticket}<button on:click={() => openTicket(ticket)}><span class="ticket-id">{ticket.id}</span><div><strong>{ticket.title}</strong><small>{ticket.department} · Due {ticket.due_date || 'Not set'}</small></div><span class="priority">{ticket.priority}</span><span class="status">{ticket.status}</span><i class="ph-bold ph-arrow-square-out"></i></button>{/each}</div>
    </div>
  {/if}
</div>
