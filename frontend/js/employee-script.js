...
/* NOTE: the New Request page (frontend/employee_NM/new-request.html) wires
   its own three submit handlers inline (submitEmployeeStandardTicket,
   submitEmployeeLeaveTicket, submitEmployeeAnonymousTicket) directly
   against its current form IDs (standardTicketForm, leaveTicketForm,
   anonymousTicketForm). A prior helper that used to live here targeted an
   older UI revision's field IDs that no longer exist - and worse, it
   still attached a second submit listener onto the current leave form
   (same element id, stale field lookups), which would have fired
   alongside the current page's own handler. Removed entirely rather than
   left as dead/conflicting code - see prefillRequestForm()
   below for how the chatbot talks to the current
   forms. */
...
function prefillRequestForm(requestType, draft) {
    ...
    if (requestType === "standard") {
        ...
        // startDate/endDate are tracked separately end-to-end now (see
        // backend/models/chatbot.py TicketDraft) - each maps to its own
        // field, never one value duplicated into both, and never
        // fabricated when the user only gave one side of the range.
        // draft.preferredDate is only a fallback for an OLD persisted
        // draft that predates startDate/endDate.
        setFieldValue("leaveStartDate", draft.startDate || draft.preferredDate);
