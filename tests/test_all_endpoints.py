...
def test_tickets_crud():
    ...
    # Comments
    res = client.post(f"/api/tickets/{t_id}/comments", json={"message": "We are inspecting your laptop.", "sender_role": "IT Staff"})
    assert res.status_code == 201
...
def test_announcements_crud():
    ...
    res = client.post("/api/announcements", json={"title": "Test Alert", "content": "Server reboot tonight.", "category": "IT Notice"})
    assert res.status_code == 201
    anc = res.json()
...
def test_notifications_crud():
    ...
    res = client.post("/api/notifications", json={"title": "Test Notif", "message": "Notification message body"})
    assert res.status_code == 201
    notif = res.json()
...
def test_onboarding_crud():
    ...
    res = client.post("/api/onboarding", json={"employee_name": "Test Candidate", "role": "QA Engineer", "department": "QA", "visa_status": "H-1B"})
    assert res.status_code == 201
    rec = res.json()
...
def test_user_profile():
    ...
    res = client.put("/api/users/profile", json={"name": "Nishita M.", "phone": "+1 555-999-0000"})
    assert res.status_code == 200
