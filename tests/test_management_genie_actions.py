...
def _candidate(id_, title, department="IT Team", priority="Medium", status="Open"):
    return TicketCandidate(id=id_, title=title, department=department, priority=priority, status=status)
...
ADMIN_USER = {"role": "Admin", "oid": "admin-1", "email": "admin@co.com", "department": "IT Team"}
HR_ADMIN_USER = {"role": "Department Admin", "oid": "hr-1", "email": "hr@co.com", "department": "HR Team"}
NO_DEPARTMENT_ADMIN_USER = {"role": "Admin", "oid": "nodept-1", "email": "nodept@co.com"}
UNMAPPABLE_DEPARTMENT_ADMIN_USER = {
    "role": "Admin", "oid": "um-1", "email": "um@co.com", "department": "Upper Executive Management"
}
...
def fake_get_all_tickets_by_department(**kw):
    ...
    fixed = [
        {"id": "HD-7001", "title": "IT hardware ticket", "department": "IT Team", "priority": "Medium", "status": "Open"},
        {"id": "HD-7002", "title": "HR onboarding ticket", "department": "HR Team", "priority": "Medium", "status": "Open"},
    ]
...
def test_reassign_ticket_unambiguous_updates_real_ticket():
    ...
    response = ask_mgmt(
        ...
        pending_action=PendingManagementAction(action_type=ChatIntent.REASSIGN_TICKET, ticket_id=ticket_id),
    )
...
def test_reassign_ticket_missing_ticket_asks_with_candidates(monkeypatch):
    candidates = [_candidate("HD-2001", "VPN connection issue"), _candidate("HD-2002", "Badge access problem")]
    monkeypatch.setattr(management_action_service, "get_all_tickets", lambda **kw: [
        {"id": c.id, "title": c.title, "department": c.department, "priority": c.priority, "status": c.status}
        for c in candidates
    ])
    called = []
    monkeypatch.setattr(management_action_service, "execute_tool", lambda *a, **kw: called.append((a, kw)) or "Success")

    decision = _decision(ChatIntent.REASSIGN_TICKET, target_department="IT Team")
    response = ask_mgmt("Move this ticket to IT", decision=decision, current_user=ADMIN_USER)
...
def test_reassign_ticket_multiple_matches_no_guessing(monkeypatch):
    candidates = [_candidate("HD-3001", "Printer offline"), _candidate("HD-3002", "Printer jam")]
    monkeypatch.setattr(management_action_service, "get_all_tickets", lambda **kw: [
        {"id": c.id, "title": c.title, "department": c.department, "priority": c.priority, "status": c.status}
        for c in candidates
    ])
    called = []
    monkeypatch.setattr(management_action_service, "execute_tool", lambda *a, **kw: called.append(1) or "Success")

    decision = _decision(ChatIntent.REASSIGN_TICKET, target_department="IT Team")
    response = ask_mgmt("Move the printer ticket to IT", decision=decision, current_user=ADMIN_USER)
...
def test_reassign_ticket_invalid_department_asks_again(monkeypatch):
    ...
    response = ask_mgmt(
        ...
        pending_action=PendingManagementAction(
            action_type=ChatIntent.REASSIGN_TICKET, ticket_id="HD-9001", ticket_title="Some ticket"
        ),
    )
    ...
    for dept in ("HR Team", "Accounting Team", "Workplace Operations Team", "IT Team", "Upper Management"):
        assert dept in response.message


def test_reassign_ticket_employee_role_denied():
    decision = _decision(ChatIntent.REASSIGN_TICKET, ticket_id="HD-1001", target_department="IT Team")
    response = ask_mgmt("Move HD-1001 to IT", decision=decision, current_user=EMPLOYEE_USER)
    assert response.pending_action is None
...
def test_reassign_ticket_member_role_denied():
    decision = _decision(ChatIntent.REASSIGN_TICKET, ticket_id="HD-1001", target_department="IT Team")
    response = ask_mgmt("Move HD-1001 to IT", decision=decision, current_user=MEMBER_USER)
    assert "isn't authorized" in response.message.lower()
...
def test_change_priority_unambiguous_updates_real_ticket():
    ...
    response = ask_mgmt(
        ...
        decision=decision,
        current_user=ADMIN_USER,
        pending_action=PendingManagementAction(action_type=ChatIntent.CHANGE_PRIORITY, ticket_id=ticket_id),
    )
    assert response.pending_action is None
    assert "Done." in response.message
...
def test_change_priority_missing_ticket_shows_current_priority(monkeypatch):
    ...
    decision = _decision(ChatIntent.CHANGE_PRIORITY, target_priority="Low")
    response = ask_mgmt("Reprioritize a ticket", decision=decision, current_user=ADMIN_USER)
...
def test_change_priority_multiple_matches_no_guessing(monkeypatch):
    candidates = [_candidate("HD-5001", "A", priority="High"), _candidate("HD-5002", "B", priority="High")]
    monkeypatch.setattr(management_action_service, "get_all_tickets", lambda **kw: [
        {"id": c.id, "title": c.title, "department": c.department, "priority": c.priority, "status": c.status}
        for c in candidates
    ])
    decision = _decision(ChatIntent.CHANGE_PRIORITY, target_priority="Low")
    response = ask_mgmt("Move the high priority one to low", decision=decision, current_user=ADMIN_USER)
    assert response.pending_action.awaiting == "ticket_selection"
    assert len(response.ticket_candidates) == 2
...
def test_change_priority_employee_role_denied():
    decision = _decision(ChatIntent.CHANGE_PRIORITY, ticket_id="HD-1001", target_priority="Low")
    response = ask_mgmt("Make HD-1001 low", decision=decision, current_user=EMPLOYEE_USER)
    assert response.pending_action is None
    assert "isn't authorized" in response.message.lower()
...
def _patch_departments(monkeypatch):
    monkeypatch.setattr(
        ...
        "list_departments",
        lambda: [{"name": n} for n in ("IT Team", "HR Team", "Accounting Team", "Upper Executive Management")],
    )
...
def test_create_employee_asks_only_missing_fields(monkeypatch):
    ...
    response = ask_mgmt("Create a new employee", decision=decision, current_user=SUPER_ADMIN_USER)
...
def test_create_employee_missing_object_id_asks_and_does_not_create(monkeypatch):
    _patch_departments(monkeypatch)
    called = []
    monkeypatch.setattr(management_action_service, "add_department_user", lambda **kw: called.append(kw))
    ...
    response = ask_mgmt("Add Priya Shah", decision=decision, current_user=SUPER_ADMIN_USER)

    assert not called
    assert response.pending_action.awaiting == "employee_field:employee_object_id"
    assert "object id" in response.message.lower() or "entra" in response.message.lower()
...
def test_create_employee_all_fields_gathered_shows_review_not_created(monkeypatch):
    ...
    response = ask_mgmt("here are all the details", decision=decision, current_user=SUPER_ADMIN_USER)
...
def test_create_employee_explicit_confirmation_creates(monkeypatch):
    ...
    decision = _decision(ChatIntent.CREATE_PORTAL_EMPLOYEE)
    response = ask_mgmt("yes, create it", decision=decision, current_user=SUPER_ADMIN_USER, pending_action=pending)
...
def test_create_employee_unauthorized_role_denied(monkeypatch):
    ...
    monkeypatch.setattr(management_action_service, "add_department_user", lambda **kw: called.append(kw))
    decision = _decision(ChatIntent.CREATE_PORTAL_EMPLOYEE)
    response = ask_mgmt("Create a new employee", decision=decision, current_user=ADMIN_USER)
    assert not called
    assert response.pending_action is None
    assert "isn't authorized" in response.message.lower()
...
def test_create_employee_never_bypasses_super_admin_requirement(monkeypatch):
    ...
    monkeypatch.setattr(management_action_service, "add_department_user", lambda **kw: called.append(kw))
    decision = _decision(ChatIntent.CREATE_PORTAL_EMPLOYEE)
    for role in ("Admin", "Operations Admin", "Department Admin", "Upper Executive Lead"):
        response = ask_mgmt(
            "Create a new employee", decision=decision, current_user={"role": role, "oid": "x"}
        )
...
def test_reassign_ticket_continuation_resolves_from_candidates(monkeypatch):
    candidates = [_candidate("HD-6001", "Badge access problem"), _candidate("HD-6002", "VPN connection issue")]
    monkeypatch.setattr(management_action_service, "get_all_tickets", lambda **kw: [
        {"id": c.id, "title": c.title, "department": c.department, "priority": c.priority, "status": c.status}
        for c in candidates
    ])
    called = []
    monkeypatch.setattr(management_action_service, "execute_tool", lambda name, args, **kw: called.append(args) or "Success: ok")
    ...
    response = ask_mgmt(
        "the VPN one", decision=decision, current_user=ADMIN_USER, pending_action=pending,
...
def test_change_priority_continuation_from_bare_ticket_id():
    ...
    pending = PendingManagementAction(
        action_type=ChatIntent.CHANGE_PRIORITY, awaiting="ticket_selection", target_priority="Low"
    )
    decision = _decision(ChatIntent.CHANGE_PRIORITY, ticket_id=ticket_id)
    response = ask_mgmt(
        ticket_id, decision=decision, current_user=ADMIN_USER, pending_action=pending,
...
def test_resolve_visibility_scope_fails_closed_for_unmappable_department():
    ...
    assert resolve_visibility_scope({"role": "Admin", "department": "Upper Executive Management"}) is None
...
def test_resolve_visibility_scope_fails_closed_for_unknown_custom_role():
    ...
    scope = resolve_visibility_scope({"role": "Regional Director", "department": "IT Team"})
...
def test_department_admin_scoped_to_it_cannot_see_hr_candidates(monkeypatch):
    monkeypatch.setattr(management_action_service, "get_all_tickets", fake_get_all_tickets_by_department)
    decision = _decision(ChatIntent.REASSIGN_TICKET, target_department="Accounting Team")
    response = ask_mgmt(
...
def test_department_admin_scoped_to_hr_cannot_mutate_it_ticket(monkeypatch):
    monkeypatch.setattr(management_action_service, "get_all_tickets", fake_get_all_tickets_by_department)
    called = []
    monkeypatch.setattr(management_action_service, "execute_tool", lambda *a, **kw: called.append(a) or "Success")

    decision = _decision(ChatIntent.REASSIGN_TICKET, ticket_id="HD-7001", target_department="Accounting Team")
    response = ask_mgmt("Move HD-7001 to Accounting", decision=decision, current_user=HR_ADMIN_USER)
...
def test_unmappable_department_scope_fails_closed_not_org_wide(monkeypatch):
    ...
    decision = _decision(ChatIntent.REASSIGN_TICKET, target_department="IT Team")
    response = ask_mgmt(
        "Move a ticket to IT", decision=decision, current_user=UNMAPPABLE_DEPARTMENT_ADMIN_USER
    )
    assert response.pending_action is None
...
def test_missing_department_scope_fails_closed(monkeypatch):
    ...
    decision = _decision(ChatIntent.CHANGE_PRIORITY, target_priority="Low")
    response = ask_mgmt(
        "Reprioritize a ticket", decision=decision, current_user=NO_DEPARTMENT_ADMIN_USER
    )
    assert response.pending_action is None
    assert response.ticket_candidates == []
    assert "couldn't determine your ticket scope" in response.message.lower()
...
def test_forged_pending_action_ticket_id_cannot_bypass_scope(monkeypatch):
    ...
    decision = _decision(ChatIntent.REASSIGN_TICKET)
    response = ask_mgmt(
        "go ahead", decision=decision, current_user=HR_ADMIN_USER, pending_action=pending
    )
...
def test_super_admin_can_see_and_mutate_ticket_outside_any_single_department(monkeypatch):
    monkeypatch.setattr(management_action_service, "get_all_tickets", fake_get_all_tickets_by_department)
    called = []
    monkeypatch.setattr(
        management_action_service, "execute_tool", lambda name, args, **kw: called.append(args) or "Success: ok"
    )
    decision = _decision(ChatIntent.REASSIGN_TICKET, ticket_id="HD-7002", target_department="Accounting Team")
    response = ask_mgmt("Move HD-7002 to Accounting", decision=decision, current_user=SUPER_ADMIN_USER)
