...
def use_mock_ai() -> bool:
    ...
    raw = os.getenv("USE_MOCK_AI", "true").strip().lower()
...
def _mock_classify(
    ...
    if not matches:
        return {
            ...
            "confidence": 0.3,
            "reason": (
                "[MOCK] No keyword rule matched this ticket; routed for manual triage."
            ),
    ...
    return {
        ...
        "reason": (
            f"[MOCK] Deterministic keyword rule matched "
            f"'{category}' under '{department}'."
        ),
